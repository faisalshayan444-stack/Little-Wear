...
const PRODUCTS = [
  {
    id: 1,
    name: "The Playtime Polo t-shirt",
    category: "boys",
    price: 800,
    image: "boys.2.png", // Added image property
    oldPrice: 1200,
    emoji: "🦕",
    bg: "#E3F2FD",
    badge: "hot",
    ...
    description: "Timeless design tailored for active days and effortless everyday wear Available in vibrant colours that stay bright wash after wash.",
    material: "Premium, breathable cotton for all-day comfort. Pre-washed for softness and durability.",
...
function addToCart(productId, size, color, qty = 1) {
  ...
  if (existing) {
    ...
    // Added image property here so cart items can also display pictures!
    cart.push({ key, id: productId, name: p.name, image: p.image, emoji: p.emoji, price: p.price, size, color, qty });
  }
  saveCart(cart);
  updateCartBadge();
}
